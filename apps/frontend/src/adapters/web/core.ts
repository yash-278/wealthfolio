...
export const invoke = async <T>(command: string, payload?: Record<string, unknown>): Promise<T> => {
  ...
  switch (command) {
    case "get_capture_reviews": {const p=payload as {page:number;pageSize:number;reason?:string}; const params=new URLSearchParams({page:String(p?.page??0),pageSize:String(p?.pageSize??25)});if(p?.reason)params.set("reason",p.reason);url+=`?${params}`;break;}
    case "create_capture_token": {body=JSON.stringify(payload);break;}
    case "revoke_capture_token": {url+=`/${encodeURIComponent((payload as {id:string}).id)}`;break;}
    case "update_capture_settings": { body = JSON.stringify((payload as { settings: unknown }).settings); break; }
    case "resolve_capture_review": { const data = payload as { id: string; resolution: unknown }; url += `/${encodeURIComponent(data.id)}/resolve`; body = JSON.stringify(data.resolution); break; }
    case "dismiss_capture_review": { const data = payload as { id: string; version: number }; url += `/${encodeURIComponent(data.id)}/dismiss`; body = JSON.stringify({ version: data.version }); break; }
    case "submit_capture": { body = JSON.stringify((payload as { input: unknown }).input); break; }
    case "retry_capture":
    case "clear_capture_source": { const data=payload as {id:string;version:number}; url+=`/${encodeURIComponent(data.id)}/${command === "retry_capture" ? "retry" : "source"}`; body=JSON.stringify({version:data.version}); break; }
    case "get_capture": { url += `/${encodeURIComponent((payload as { id: string }).id)}`; break; }
    case "update_account": {
